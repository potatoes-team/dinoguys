export default class UsernameSceneConfig {
	...
	createTypingText(x, y, config) {
		...
		const { scene } = this;
		const { isBackground, bgColor, strokeColor, fixedWidth, fixedHeight, fontFamily, fontSize } = config;
		...
		const textBox = scene.rexUI.add
			.textBox({
				...
				background: isBackground // if background color is true, create roundRectangle background
					...
					: undefined,
				text: this.getText('', 0, fixedWidth, fixedHeight, fontFamily, fontSize), // start the text off as an empty string
				orientation: 0,
				space: {
					left: 20,
					right: 20,
	...
	getText(text, wrapWidth, fixedWidth, fixedHeight, fontFamily, fontSize = '22px') {
	...
	addButtons(customFont) {
		...
		this.state.confirmationButtons = scene.rexUI.add
			.buttons({
				...
				y: 550,
				orientation: 0,
				buttons: [this.createConfirmationButton('yes', customFont), this.createConfirmationButton('no', customFont)],
				space: {
