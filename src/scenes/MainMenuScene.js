export default class MainMenuScene extends Phaser.Scene {
	...
	create() {
		...
		this.add.text(width / 2, height / 4, 'Main Menu Scene', { fontSize: '32px' }).setOrigin(0.5, 0.5);
    ...
    singlePlayerBtn.on('pointerup', () => {
      ...
      this.scene.start('CharSelection', { isMultiplayer: false, menuMusic: this.menuMusic} );
    });

    multiplayerBtn.on('pointerup', () => {
      this.scene.stop('MainMenuScene');
      this.scene.start('CharSelection', { socket: this.socket, isMultiplayer: true, username: this.username, menuMusic: this.menuMusic});
    });
  }
}
