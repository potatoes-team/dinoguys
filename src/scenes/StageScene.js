...
export default class StageScene extends Phaser.Scene {
	...
	create() {
		...
		if (this.isMultiplayer) {
			...
			this.playerCountdown = this.add
				.text(this.scale.width / 2, this.scale.height / 2, `Waiting for all players loaded...`, {
					fontSize: '30px',
				})
				.setOrigin(0.5, 0.5)
				.setScrollFactor(0);
			...
			this.socket.on('updateWinners', (winnerNum) => {
				this.stageLimitText.setText(`Stage Limit: ${winnerNum}/${this.stageLimit}`);
			});
			...
			this.socket.on('playerLeft', ({ playerId, newStageLimits, winnerNum }) => {
				if (this.opponents[playerId]) {
	...
	update() {
		...
		if (!this.hurt) {
			...
			if (
				...
				(this.stageKey === 'StageForest' && this.player.y >= this.scale.height - 50)
			) {
				this.player.respawn();
			}
		}
	...
	createParallaxBackgrounds() {
		...
		for (let i = 0; i < bg.layerNum; ++i) {
			this.add
				.tileSprite(0, height, width * bg.tileWidth[i], bg.imgHeight, `${this.assetName}_bgLayer${i + 1}`)
				.setOrigin(0, 1)
				.setScale(bg.scale)
	...
	createGoalFlag() {
		// create flag
		this.flag = this.physics.add.staticSprite(this.endPoint.x, this.endPoint.y, 'flag').setOrigin(0.5, 1);
