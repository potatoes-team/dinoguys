...
export default class WaitingScene extends Phaser.Scene {
	...
	create() {
		...
		// create player
		this.player = new player(this, 20, 400, this.charSpriteKey, this.username, this.socket, this.platform).setScale(
			2.25
		);
		...
		Object.keys(this.roomInfo.players).forEach((playerId) => {
			if (playerId !== this.socket.id) {
				...
				this.opponents[playerId] = new player(this, 20, 400, spriteKey, username, this.socket, this.platform);
				this[`opponents${playerId}`] = this.add
					.text(this.opponents[playerId].x, this.opponents[playerId].y - 16, username, {
						fontSize: '10px',
						fill: '#fff',
					})
					.setOrigin(0.5, 1);
		...
		this.playerCounter = this.add.text(470, 40, `${this.roomInfo.playerNum} player(s) in lobby`, {
			fontSize: '30px',
			fill: '#fff',
		});
		...
		this.socket.on('newPlayerJoined', ({ playerId, playerInfo }) => {
			...
			this.waitingForPlayers.setText(`Waiting for ${this.requiredPlayers - this.roomInfo.playerNum} player(s)`);
			this.playerCounter.setText(`${this.roomInfo.playerNum} player(s) in lobby`);

			this[`opponents${playerId}`] = this.add
				.text(this.opponents[playerId].x, this.opponents[playerId].y - 16, this.roomInfo.players[playerId].username, {
					fontSize: '10px',
					fill: '#fff',
				})
				.setOrigin(0.5, 1);
